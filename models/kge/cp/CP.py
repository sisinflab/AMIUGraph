...
class CP(KgeModel):
    def __init__(self, *args, **kwargs):
        ...
        self.emb_set_node_h = torch.nn.Parameter(torch.nn.init.xavier_normal_(
            torch.empty((self.num_set_node, self.emb_size))).to(self.device))

        self.emb_set_node_t = torch.nn.Parameter(torch.nn.init.xavier_normal_(
            torch.empty((self.num_set_node, self.emb_size))).to(self.device))
        ...
        if len(self.inductive_embs) > 0:
            ...
            self.inductive_proj_h = torch.nn.Linear(self.inductive_embs.shape[-1], self.emb_size).to(torch.float32).to(self.device)
            self.inductive_proj_t = torch.nn.Linear(self.inductive_embs.shape[-1], self.emb_size).to(torch.float32).to(self.device)
            self.set_trainable_params([self.emb_set_node_h, self.emb_set_node_t, self.emb_relations,
                                       *list(self.inductive_proj_h.parameters()),
                                       *list(self.inductive_proj_t.parameters()),
                                       self.inductive_embs])
        else:
            self.set_trainable_params([self.emb_set_node_h, self.emb_set_node_t, self.emb_relations])
    ...
    def lookup(self, inputs):
        """
        def lookup(self, inputs):
            head = self.emb_set_node_h[inputs[0]]
            rel = self.emb_relations[inputs[2]]
            tail = self.emb_set_node_t[inputs[1]]
        ...
        if len(head_inductive_indices) > 0:
            ...
            head[head_inductive_indices] = self.inductive_proj_h(self.inductive_embs[ind_head_idx])
        ...
        if len(tail_inductive_indices) > 0:
            ...
            tail[tail_inductive_indices] = self.inductive_proj_t(self.inductive_embs[ind_tail_idx])

        # Assign regular embeddings for head and tail
        if len(head_regular_indices) > 0:
            head[head_regular_indices] = self.emb_set_node_h[head_input[head_regular_indices]]

        if len(tail_regular_indices) > 0:
            tail[tail_regular_indices] = self.emb_set_node_t[tail_input[tail_regular_indices]]
    ...
    def score(self, head, tail, rel):
        out = torch.sigmoid(torch.sum(head * rel * tail, dim=-1))
        return out
